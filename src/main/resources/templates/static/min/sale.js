...
function to_page(a) {
    $.ajax({
        type: "POST", url: "/admin/v1/book/sale", data: "pn=" + a, error: function (a) {
...
function build_book_table(a) {
    ...
    $.each(a.data.page.list, function (a, b) {
        var d = $("<td></td>").append(a + 1), c = $("<td></td>").append(b.book_uid).attr("id", "id_" + a),
            e = $("<td></td>").append("\uffe5" + b.total_price), g = $("<td></td>").append(b.nums);
        $("<tr></tr>").append(d).append(c).append(e).append(g).appendTo("tbody");
        getBook("id_" + a, b.book_uid)
    })
}
